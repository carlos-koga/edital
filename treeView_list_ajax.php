...
try {
    ...
    if ( $tipoEdital == 0) {
        $stmt = $pdo->prepare("SELECT cls_nu, level, cls_tx , ativo FROM banco_clausula ORDER BY cls_nu_ordem");
        $stmt->execute();
    } else {
        $stmt = $pdo->prepare("SELECT B.cls_nu, level, cls_tx , E.cls_nu AS chk, B.ativo FROM banco_clausula AS B LEFT JOIN (SELECT * FROM edital_clausula WHERE edt_id = :editalId ) E ON B.cls_nu = E.cls_nu ORDER BY cls_nu_ordem");
        $stmt->execute([':editalId' => $tipoEdital]);

        $stmt2 = $pdo->prepare("SELECT edt_bloqueado FROM edital WHERE edt_id = :editalId");
