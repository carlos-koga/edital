...
try {
    ...
    if ($oper == 'inc') {
        $stmt = $pdo->prepare("SELECT cls_nu_ordem FROM banco_clausula WHERE cls_nu = :id_atual");
        $stmt->execute([':id_atual' => $idAtual]);
        $ordem_atual = $stmt->fetchColumn();
        echo $ordem_atual;

        $stmt = $pdo->prepare("SELECT cls_nu_ordem FROM banco_clausula WHERE cls_nu_ordem > :nivel_atual ORDER BY cls_nu_ordem LIMIT 1");
        ...
        try {
            ...
            $stmt = $pdo->prepare("UPDATE banco_clausula SET cls_nu_ordem = cls_nu_ordem + 10 WHERE cls_nu_ordem >= :next_order");
            $stmt->execute([':next_order' => $next_order]);
            $stmt = $pdo->prepare("INSERT INTO banco_clausula (cls_tx, cls_nu_ordem, level) VALUES(:clausula, :ordem, :nivel)");
            $stmt->execute([':clausula' => $texto, ':ordem' => $next_order, ':nivel' => $nivel]);
            $clsNu = $pdo->lastInsertId();
            $pdo->commit();
            $rowId = "row$clsNu";
        ...
        $stmt = $pdo->prepare("UPDATE banco_clausula SET cls_tx=:clausula, level=:nivel  WHERE cls_nu = :id_atual");
        $stmt->execute([':clausula' => $texto, ':nivel' => $nivel, ':id_atual' => $idAtual]);
    } elseif ($oper == 'del') {
        $stmt = $pdo->prepare("DELETE FROM banco_clausula WHERE cls_nu = :id_atual");
        $stmt->execute([':id_atual' => $idAtual]);
        $stmt = $pdo->prepare("DELETE FROM edital_clausula WHERE cls_nu = :id_atual");
        $stmt->execute([':id_atual' => $idAtual]);
    } elseif ($oper == 'desativar') {
        $stmt = $pdo->prepare("UPDATE banco_clausula SET ativo = 0 WHERE cls_nu = :id_atual");
        $stmt->execute([':id_atual' => $idAtual]);
    } elseif ($oper == 'chk') {
        $estado = $_POST['estado'];
        ...
        if ($estado == 'true') {
            $stmt = $pdo->prepare("INSERT INTO edital_clausula (edt_id, cls_nu) VALUES (:idEdital , :id_atual)");
            $stmt->execute([':idEdital' => $idEdital, ':id_atual' => $idAtual]);
        } elseif ($estado == 'false') {
            $stmt = $pdo->prepare("DELETE FROM edital_clausula WHERE edt_id = :idEdital  AND cls_nu = :id_atual");
            $stmt->execute([':idEdital' => $idEdital, ':id_atual' => $idAtual]);
        }
    }
