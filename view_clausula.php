                ...
                <tr>
                    <td>
                        ...
                        try {
                            ...
                            if ($idEdital != 0) {
                                $stmt = $pdo->prepare("SELECT  edt_nome || '_' || edt_versao || '.' || edt_designacao AS nome FROM edital WHERE edt_id = :idEdital");
                            ...
                            // Consulta os dados
                            if ($idEdital != 0) {
                                $stmt = $pdo->prepare("SELECT B.cls_nu, B.cls_tx FROM banco_clausula AS B INNER JOIN (SELECT cls_nu FROM edital_clausula WHERE edt_id = :idEdital) AS E ON B.cls_nu = E.cls_nu ORDER BY cls_nu_ordem");
                                $stmt->execute([':idEdital' => $idEdital]);
                            } else {
                                $stmt = $pdo->prepare("SELECT cls_nu, cls_tx FROM banco_clausula ORDER BY cls_nu_ordem");
                                $stmt->execute();
                            }
